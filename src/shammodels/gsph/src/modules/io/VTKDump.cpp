...
namespace shammodels::gsph::modules {
    ...
    void VTKDump<Tvec, SPHKernel>::do_dump(std::string filename, bool add_patch_world_id) {
        ...
        // restarts)
        const u32 idensity    = pdl.get_field_idx<Tscal>(newtonian::density);
        const u32 ipressure   = pdl.get_field_idx<Tscal>(newtonian::pressure);
        const u32 isoundspeed = pdl.get_field_idx<Tscal>(newtonian::soundspeed);
