...
sedov_analytical = SedovAnalytical(
    gamma=gamma,
    E_blast=E_blast,
    rho_0=rho_0,
    ndim=3
...
fig.suptitle('GSPH Sedov-Taylor Blast Wave - Comparison with Analytical Solution',
...
def update(frame_num):
    ...
    ax1.scatter(r_sim[mask], rho_sim[mask], color=sim_color, s=15, alpha=0.6, label='GSPH', zorder=2)
    ...
    ax1.axvline(sedov_analytical.shock_radius(time), color='gray', linestyle='--',
                alpha=0.5, label=f'$R_s$ = {sedov_analytical.shock_radius(time):.3f}')

    # Velocity
    ax2.plot(r_ana, vel_ana, color=ana_color, linewidth=2.5, label='Analytical', zorder=1)
    ax2.scatter(r_sim[mask], vel_sim[mask], color=sim_color, s=15, alpha=0.6, label='GSPH', zorder=2)
    ...
    ax3.scatter(r_sim[mask], pres_sim[mask], color=sim_color, s=15, alpha=0.6, label='GSPH', zorder=2)
    ...
    ax4.text(0.5, 0.5, f'Shock Radius: $R_s$ = {sedov_analytical.shock_radius(time):.4f}\n\n'
    ...
    # Add time label
    fig.suptitle(f'GSPH Sedov-Taylor Blast Wave - t = {time:.4f}\n'
...
if HAS_ANIMATION and len(frame_data) > 0:
    anim = FuncAnimation(fig, update, frames=len(frame_data),
...
if len(frame_data) > 0:
    ...
    fig2.suptitle(f'GSPH Sedov-Taylor Blast Wave - Final State (t = {time:.4f})\n'
    ...
    # Info panel
    info_text = (f'Sedov-Taylor Parameters:\n\n'
                 f'$\\gamma$ = {gamma:.4f}\n'
    ...
    ax4.text(0.5, 0.5, info_text, transform=ax4.transAxes, fontsize=12,
