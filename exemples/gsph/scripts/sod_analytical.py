...
class SodAnalytical:
    ...
    def __init__(self, gamma=1.4, rho_L=1.0, rho_R=0.125,
                 p_L=1.0, p_R=0.1, u_L=0.0, u_R=0.0, x0=0.0):
        self.gamma = gamma
        self.rho_L = rho_L
        self.rho_R = rho_R
        self.p_L = p_L
    ...
    def _solve_star_region(self):
        ...
        p_star = 0.5 * (self.p_L + self.p_R) - \
                 0.125 * (self.u_R - self.u_L) * (self.rho_L + self.rho_R) * (self.c_L + self.c_R)
        ...
        for _ in range(max_iter):
            ...
            if p_star > self.p_L:
                ...
                # Rarefaction
                f_L = 2.0 * self.c_L / (gamma - 1) * \
                      ((p_star / self.p_L) ** ((gamma - 1) / (2 * gamma)) - 1.0)
                df_L = 1.0 / (self.rho_L * self.c_L) * \
            ...
            if p_star > self.p_R:
                ...
                # Rarefaction
                f_R = 2.0 * self.c_R / (gamma - 1) * \
                      ((p_star / self.p_R) ** ((gamma - 1) / (2 * gamma)) - 1.0)
                df_R = 1.0 / (self.rho_R * self.c_R) * \
    ...
    def _compute_wave_speeds(self):
        ...
        if self.p_star > self.p_L:
            ...
            self.S_L = self.u_L - self.c_L * np.sqrt(
                (gamma + 1) / (2 * gamma) * self.p_star / self.p_L +
                (gamma - 1) / (2 * gamma))
        ...
        if self.p_star > self.p_R:
            ...
            self.S_R = self.u_R + self.c_R * np.sqrt(
                (gamma + 1) / (2 * gamma) * self.p_star / self.p_R +
                (gamma - 1) / (2 * gamma))
...
class SedovAnalytical:
    ...
    def solution_at_time(self, t, n_points=500):
        ...
        if t <= 1e-10:
            ...
            return r, np.ones(n_points) * self.rho0, np.zeros(n_points), \
                   np.ones(n_points) * 1e-10, np.ones(n_points) * 1e-10
